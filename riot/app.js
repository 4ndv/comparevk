...
fetch = function(group) {
  ...
  return VK.Api.call('execute.massfetch', {
    where: group.id,
    start: group.offset,
    fields: 'photo_50',
    amount: '15000',
...
merge = function() {
  var c, first, firstids, inters, second, secondids;
  ...
  firstids = first.map(function(item) {
    return item.id;
  });
...
self.process = function(e) {
  ...
  return VK.Api.call('groups.getById', {
    group_ids: [self.first.value, self.second.value]
  }, function(r) {
    if (r.error || !r.response || (r.response.length !== 2) || (self.first.value === self.second.value)) {
      return self.update({
        error: true,
        working: null
      });
      ...
      return r.response.forEach(function(item, index, array) {
        self.members[index] = [];
        return fetch({
          index: index,
          id: item.gid,
